...
- [ ] P0: Close the remaining CPU decode gap against the strongest controlled CPU-only llama.cpp reference:
      ...
      gate with `-0.79%` median overhead, `98.44%` coverage, and at most `5.61%` stage CV. It measures reference
      activation/Down at `0.210/43.045 ms/token` versus LiteNN helper lower bounds of `11.043/42.723 ms/token`.
      Activation therefore owns the `10.833 ms` confirmed deficit; no Down deficit is established. The implementation
      checklist is maintained in
      `docs/PerformanceOptimizationRoadmap.md` under the 2026-08-04 CPU decode gap closure tranche.
      - [ ] Follow the evidence-backed order: use the completed low-overhead reference split, closed
            RMSNorm-to-grouped-Q8_K rejection, and rejected Down-kernel tranche; select and validate a bounded vector
            SwiGLU policy; reproduce the external `6.85 token/s` provenance; then extend controls to sustained decode
            and long-context tiers. Rejected synchronized stage deltas and cache-hot-only wins must not reorder this
            sequence.
      ...
      - [x] Split the controlled reference's SwiGLU activation from Q4_K/Q6_K Down with non-synchronizing aggregate
            counters. Three exact-token clean/profile pairs passed all gates: `-0.79%` median overhead, `98.44%`
            coverage, `2.66/0.25%` clean/profile whole-run CV, and at most `5.61%` stage CV.
      ...
      - [x] P0 parallel: attribute the sustained context-dependent module slope before selecting Attention/KV
            ...
            - [ ] Localize the first natural logit mismatch with per-layer checkpoints. It occurs at generated index
                  ...
                  - [x] P0: expose and compare selectable sub-layer boundaries in blocks 31-33 and 43-47. The 13-boundary
                        ...
                        The standalone data report is `docs/QwenSubLayerDriftAnalysis_2026-08-12.md`.
                  ...
                  - [ ] P1: establish a distributional quality gate before changing model math.
                        ...
                        - [x] P0: measure corpus perplexity/cross-entropy delta on a fixed public evaluation slice and
                              ...
                              The predeclared 3-sample/192-target gate passes: reference/LiteNN cross-entropy is
                              `3.281490/3.272574 nats`, the delta is `-0.008916 nats`, relative perplexity delta is
                              `-0.887681%`, worst sample regression is `0.001057 nats`, and fallback/non-finite counts
                              are zero. Evidence: `docs/QwenCorpusLossAnalysis_2026-08-14.md`.
                              - [x] Freeze a license-compatible public text-slice manifest with stable sample ids,
                              ...
                              - [x] Add an explicit llama.cpp teacher-forced pre-target logit capture. The existing
                              ...
                              - [x] Orchestrate identical BOS/EOS, prompt/target split, context reset, token coverage,
                              ...
                              - [x] Compute stable log-sum-exp NLL for the actual next token, per-token and per-sample
                              ...
                              - [x] Add synthetic evaluator tests and execute the real public slice. Only a material
                                    corpus-loss failure promotes explanation step 33 to same-input projection/sub-layer
                                    localization; the passing result retains the measured drift as an accepted residual.
      ...
      - [ ] P0: reduce the confirmed FFN activation deficit; keep Down rewrites closed without new accepted evidence.
            ...
            - [x] Add a real imported-plan/generated-artifact regression for SwiGLU-to-field-v4 Down fusion. The
                  source plan and prepared layout are eligible; stale cache-v2 object code caused the profiled
                  standalone calls. Cache version 3 now invalidates it and the fresh artifact imports the fused helper.
