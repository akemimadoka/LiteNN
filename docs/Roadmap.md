...
- [x] `Im2ColNode`: 通用滑窗展开（1D/2D/3D 由 spatial rank 参数决定）。
      ...
      滑窗展开与零 padding；Validator/Dump/ModelIO v15/Interpreter/ConstFold/pass
...
- [x] `Conv2DNode`: 直接卷积原语，与 Im2Col-Layer 路径互为参考；CUDA 后端绑定 cuDNN。
      ...
      dilation、padding、可选 bias、grouped convolution；Validator/Dump/ModelIO v15/Interpreter/
...
- [x] `Pool2DNode`: max/average 池化，含 1D 退化形式（kernel 高度 = 1 即 Pool1D）。
      ...
      channel-first 4D 输入、padding、average `countIncludePad` 语义；Validator/Dump/ModelIO v15/
...
- [ ] G2.2 P2 `PAD`、`CUMSUM`、`REPEAT`：由 G5.1 / G5.2 的新 Node 直接驱动 Layer 重写。
      （已完成：`PAD` 与 `CUMSUM`；`REPEAT` 仍待 `BroadcastTo + Reshape` 或专用 Layer 收口。）
...
- [ ] G2.2 P2 `CONV_1D/2D/3D`、`CONV_TRANSPOSE_*`、`IM2COL`、`POOL_*`、`UPSCALE`：由 G5.4 驱动。
      （已完成：`IM2COL` 与 `POOL_*` 的 CPU/reference Node；`CONV_*`、`CONV_TRANSPOSE_*`、
      `UPSCALE` 仍待推进；`CONV_2D` 已有 CPU/reference `Conv2DNode`，`CONV_1D/3D`
      可先通过维度适配或后续专用 Node 收口。）
- [ ] G2.2 P2 `SSM_CONV`、`SSM_SCAN`：由 G5.2 `SSMScanNode` 驱动，等待真实 Mamba 目标模型。
      （已完成：`SSM_SCAN` 的最小 CPU reference Node；`SSM_CONV` 与真实模型参数布局待补。）
- [ ] G2.2 P2 `RWKV_WKV6/7`、`GATED_LINEAR_ATTN`、`GATED_DELTA_NET`：由 G5.2 `RWKVWKVNode` 驱动，
      等待真实 RWKV 目标模型。
      （已完成：`RWKVWKVNode` 最小 CPU reference；WKV6/7、GLA、GatedDeltaNet 真实变体映射待补。）
...
- Started G5.4 with `Im2ColNode`, `Conv2DNode`, and `Pool2DNode`: added CPU reference execution, Layer helpers,
  validation, dump, ModelIO v15 serialization, ConstFold support, pass clone/dependency plumbing, and
  `ConvolutionPoolingNodeTest` coverage.
