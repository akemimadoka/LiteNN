...
- [x] Add a token-id decode-loop smoke CLI: `--run-llama-decode-loop-token-id` repeatedly executes static decode graphs,
      carries updated KV-cache tensors between steps, and reports or writes the generated token-id sequence plus
      tokenizer pieces when `tokenizer.ggml.tokens` is available.
