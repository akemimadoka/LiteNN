...
- [x] **CUDA native AOT MVP**：`Compiler<CUDA>` 已识别静态 shape、单 subgraph、`Float32` 单输入单输出 `UnaryOp::Negate/Abs/Sqrt`、双输入单输出 `BinaryOp::Add/Subtract/Multiply/Divide`、同 rank 静态 broadcast 和二维 `Float32` MatMul；elementwise 生成 `CUDANative` PTX payload，MatMul 生成 cuBLAS library-call payload。`CompiledModule<CUDA>` 直接以 CUDA Tensor 指针 launch kernel 或调用 cuBLAS，不匹配图仍回退 CPU AOT bridge。
- [x] **CUDA PTX 模板收敛（R0/R2/R3 完成）**：elementwise/broadcast PTX 已从 `CompiledModule.cpp` 拆到独立 `CUDANativeCodegen` 模块；当前 `UnaryOp::Negate/Abs/Sqrt`、same-shape `BinaryOp::Add/Subtract/Multiply/Divide` 与 same-rank broadcast binary 已改为 `OpBuilder` 结构化生成 MLIR `gpu.module`/`gpu.func`，再 lowering 到 NVVM/LLVM IR，由 NVPTX 后端生成 PTX 并接入 CUDA native payload。`Abs/Sqrt` 采用 NVVM intrinsic，避免 standalone PTX 依赖 libdevice 外部符号；手写 PTX 暂作为 fallback 保留，后续继续扩展 reduce/fusion。详细拆分见 `docs/CUDAAOTRoadmap.md` 的 P2.5。
