...
- **CUDA**：通过 `LITENN_ENABLE_CUDA` 可选启用。当前第一版提供 CUDA Runtime 内存管理、CPU/GPU/D2D 拷贝、`PolymorphicDevice` 实例级比较与完整 Device concept 覆盖；`Float32/Float64` 二维 `MatMul` 使用 cuBLAS，其他 unary/binary/reduce/concat/slice 先走 CPU 语义回退，后续按 elementwise/reduce/fusion 逐步替换为 CUDA kernel
...
`CompiledModule<CPU>` 是当前原生 AOT 运行时封装；`CompiledModule<CUDA>` 是第一版 CUDA AOT bridge，用 CUDA Tensor 作为公开输入/输出边界，但内部复用 CPU AOT image/JIT 执行。
...
- `Compiler<CUDA>::CompileArtifact(graph)` 当前复用 CPU artifact；`Compiler<CUDA>::Compile(graph, CUDA{})` 返回 `CompiledModule<CUDA>`，在 `Run` / `RunInto` / `RunManyInto` 中校验 CUDA Tensor 签名，将输入复制到 CPU AOT module 执行，再把输出复制回指定 CUDA device
...
- `CompiledModule<CUDA>::Load({rodata, instructions}, CUDA{})` 与 CPU loader 使用相同 image 生命周期规则；当前 native instructions 仍是 CPU object bytes，不包含 GPU kernel
...
- [x] **CUDA native AOT MVP**：`Compiler<CUDA>` 已识别静态 shape、单 subgraph、`Float32` 双输入单输出 `BinaryOp::Add`，生成 `CUDANative` PTX payload；`CompiledModule<CUDA>` 通过 CUDA Driver module 直接以 CUDA Tensor 指针 launch kernel，不匹配图仍回退 CPU AOT bridge。
...
- **AOT Compiler\<D\>**：将 Graph 编译为设备特定可执行代码（`CompiledModule<D>`）
  ...
  - [x] **CUDA native loader + codegen MVP**：`CompiledModule<CUDA>` 可加载 `CUDANative` image，反序列化 payload、加载 Driver module 并按 launch table 执行；`Compiler<CUDA>` 已支持静态 shape、单 subgraph、`Float32` elementwise Add PTX codegen
  - [ ] **CUDA 原生 AOT codegen 扩展**：需要补 MLIR GPU/NVVM lowering 或更完整的独立 kernel generator、更多 elementwise/broadcast/unary/reduce、MatMul/cuBLAS/fusion epilogue、workspace allocator、stream/async 生命周期；详细分解见 `docs/CUDAAOTRoadmap.md`
