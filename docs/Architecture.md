...
└── LiteNN/
    ...
    │   └── CUDA.cpp              // CUDA Runtime 分配/拷贝、cuBLAS MatMul 与 CPU fallback op 实现
...
- [x] **CompiledModule image ABI 版本化**：rodata 包含 magic、version、pointer size、endianness、target triple、backend 和命名 input/output specs；`Load` 会进行兼容性检查并拒绝 ABI 不匹配 image。instruction 当前仍是 CPU native object bytes，依赖当前 object loader；CUDA native binary bundle 见 `docs/CUDAAOTRoadmap.md`。
...
- **AOT Compiler\<D\>**：将 Graph 编译为设备特定可执行代码（`CompiledModule<D>`）
  ...
  - [x] **CUDA AOT bridge 第一版**：`Compiler<CUDA>::Compile/CompileArtifact` 与 `CompiledModule<CUDA>::Load/Run/RunInto/RunManyInto` 已打通，支持 CUDA Tensor 输入输出、rodata/instruction 地址加载和 artifact 加载；当前执行路径复用 CPU AOT object，再通过 CPU↔CUDA copy 做边界转换
  - [x] **CUDA AOT image backend 元数据**：rodata v3 新增 `CompiledModuleBackend`，并通过 `CompiledModuleArtifact::Backend()` / `CompiledModule<D>::Backend()` 暴露；当前 bridge artifact 仍标记为 `CPUNative`
  - [ ] **CUDA 原生 AOT codegen**：需要补 MLIR GPU/NVVM lowering、PTX/cubin 或 fatbin 打包、CUDA Driver module loading、kernel launch ABI、stream/allocator 生命周期；详细分解见 `docs/CUDAAOTRoadmap.md`
