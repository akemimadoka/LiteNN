...
- **CUDA**：通过 `LITENN_ENABLE_CUDA` 可选启用。当前第一版提供 CUDA Runtime 内存管理、CPU/GPU 拷贝、`PolymorphicDevice` 实例级比较与完整 Device concept 覆盖；算子执行先走 CPU 语义回退，后续按 MatMul/elementwise/reduce 逐步替换为 CUDA kernel
...
└── LiteNN/
    ...
    │   └── CUDA.cpp              // CUDA Runtime 分配/拷贝与 CPU fallback op 实现
...
- [x] **CUDA Device 第一版**：新增 `LITENN_ENABLE_CUDA`，提供 `CUDA` device、`CUDADeviceCount()` / `IsCUDADeviceAvailable()`、CUDA Runtime 分配/释放/清零、CPU↔GPU/D2D 拷贝、dtype 转换与 unary/binary/reduce/concat/slice CPU fallback；新增 `CUDADeviceTest.cpp` 覆盖 round-trip、MatMul fallback 与 PolymorphicDevice 路径
