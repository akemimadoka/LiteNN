...
- [ ] **Autograd 完整性回归**：系统覆盖广播梯度、变量梯度顺序、多变量、多输出、共享 callee、CondNode/WhileNode、Concat/Slice、Reduce、FusedOpNode 相关组合。已新增 `AutogradRegressionTest` 覆盖广播参数梯度 reduce 回原 shape，以及 `backward results = [grad_inputs..., grad_variables...]` 中 variable gradient 按 variableIndex 排序的约定；后续还需继续补多输出和更多组合路径。
