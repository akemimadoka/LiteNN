...
- [x] **CUDA PTX 模板收敛（R0 完成，R2 部分验证）**：elementwise/broadcast PTX 仍保留 MVP 字符串模板，但已从 `CompiledModule.cpp` 拆到独立 `CUDANativeCodegen` 模块；当前 `UnaryOp::Negate` 已可通过 MLIR LLVM/NVVM dialect 生成 NVPTX PTX 并接入 CUDA native payload，后续补 `gpu.func`/`gpu.module` lowering 版本并逐步替换更多 unary/binary/broadcast。详细拆分见 `docs/CUDAAOTRoadmap.md` 的 P2.5。
...
- **AOT Compiler\<D\>**：将 Graph 编译为设备特定可执行代码（`CompiledModule<D>`）
  ...
  - [ ] **CUDA 原生 AOT codegen 扩展**：需要补 MLIR GPU/NVVM lowering 或更完整的独立 kernel generator、更多 elementwise/unary/reduce、自定义 MatMul/fusion epilogue、workspace allocator、stream/async 生命周期；详细分解见 `docs/CUDAAOTRoadmap.md`
