...
当前 Trainer 仍是 CPU + eager training API；参数组、学习率调度、batch loss 聚合和 checkpoint 训练循环留在后续 P1/P2。
...
- 文件包含 magic、format version、forward/backward 入口、Variable data、ActivationSlot/TapeSlot、所有 Subgraph 和节点 payload
...
└── LiteNN/
    ...
    │   ├── Activation.h         // BuildReLU/BuildSigmoid/BuildTanh
...
- [x] **模型保存/加载**：新增 `LiteNN::Serialization::SaveModel/LoadModel`，序列化 magic/version、forward/backward 入口、Variable data、ActivationSlot/TapeSlot、Subgraph、NodeVariant payload，并在加载后运行 `ValidateGraph`。当前已支持本库内部 checkpoint/推理模型 roundtrip；输入输出命名、格式迁移和跨版本兼容策略仍在后续条目跟踪。
