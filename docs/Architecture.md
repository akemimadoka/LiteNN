...
- [x] **CUDA PTX 模板收敛（R0/R2/R3 完成）**：elementwise/broadcast PTX 已从 `CompiledModule.cpp` 拆到独立 `CUDANativeCodegen` 模块；当前 `UnaryOp::Negate/Abs/Sqrt`、same-shape `BinaryOp::Add/Subtract/Multiply/Divide` 与 same-rank broadcast binary 已可通过 MLIR `gpu.module`/`gpu.func` lowering 到 NVVM/LLVM IR，再由 NVPTX 后端生成 PTX 并接入 CUDA native payload。`Abs/Sqrt` 采用 NVVM intrinsic，避免 standalone PTX 依赖 libdevice 外部符号；手写 PTX 暂作为 fallback 保留，后续继续扩展 reduce/fusion。详细拆分见 `docs/CUDAAOTRoadmap.md` 的 P2.5。
