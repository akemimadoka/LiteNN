...
LiteNN 是一个使用 **C++26** 实现的神经网络库，采用**编译器前端**风格进行设计。核心思想是将神经网络计算表示为一个静态计算图（Graph），通过 Pass 系统对其进行变换和优化，最终由后端执行。目前实现了基于解释执行的运行时（Interpreter）、CPU AOT 编译后端（MLIR/LLVM）和第一版 CUDA Device/AOT bridge。
...
Compiler<CUDA>             ← 当前实现（CUDA Tensor 边界 + CPU AOT bridge）
  ↓
...
- **CPU**：完整参考实现，支持所有 Op 和广播
- **CUDA**：通过 `LITENN_ENABLE_CUDA` 可选启用。当前第一版提供 CUDA Runtime 内存管理、CPU/GPU/D2D 拷贝、`PolymorphicDevice` 实例级比较与完整 Device concept 覆盖；`Float32/Float64` 二维 `MatMul` 使用 cuBLAS，Device eager 路径中的其他 unary/binary/reduce/concat/slice 先走 CPU 语义回退。CUDA AOT 已有静态 `Float32` elementwise Negate/Abs/Sqrt/Add/Subtract/Multiply/Divide 和同 rank broadcast native PTX MVP，并支持二维 `Float32` MatMul/cuBLAS library call，后续继续按 reduce/fusion 扩展
...
- `CompilerOptions` 是编译行为的显式配置面，覆盖 CPU AOT thread count、worker affinity policy、parallel min-FLOPs、LLVM opt level、编译阶段诊断、CPU external regions、generic MLIR 外置常量最小字节数、external-region fusion 和 CUDA native AOT enablement；库编译路径不直接读取环境变量，CLI/benchmark/example 若需要环境变量控制，应在入口层读取并填充该配置对象
...
- 启用 `CompilerOptions::enableCPUAOTExternalRegions` 时，CPU AOT 会优先使用优化的 f32 linear-chain 外置区路径；若不匹配，则 generic MLIR 路径可把 `VariableRefNode` 与超过 `cpuAOTExternalConstantMinBytes` 的 byte-addressable `ConstantNode` 改写为隐藏参数，由统一 entry wrapper 从 separated weights/constants region 绑定运行时 memref。该 generic rewrite 会重建子图以让 hidden params 保持拓扑前置，并支持通过 `CallNode` 与 `CondNode` 传播；如果 `WhileNode` 或特殊 fused body 需要捕获外置 tensor，则回退到普通 inline AOT，避免改变循环 carry/fusion ABI。
...
- `CompiledModuleArtifact::WriteObjectFile()` 生成 carrier object，导出 `<prefix>_rodata`, `<prefix>_rodata_size`, `<prefix>_instructions`, `<prefix>_instructions_size`；`CompiledModule::WriteObjectFile()` 保留为兼容便捷接口
...
- 生命周期约束：artifact/image 负责字节所有权，loaded module 负责 JIT engine/context 所有权；`Run`/`RunInto`/`RunManyInto` 可并发，但 `Load`、赋值、析构仍应作为生命周期操作与并发 `Run` 分离。如果通过插件或共享库边界承载 LLVM，建议在卸载对应库之前先销毁 `CompiledModule`
...
- **CPU AOT MatMul micro-kernel**：当前 codegen pass 会识别 bufferized `f32` MatMul contraction。宽输出使用 `vector<16xf32>`，按形状依次尝试 `8-row x 2-vector`、`4-row x 4-vector`、`2-row x 8-vector` tile 以复用 RHS/B；窄输出 `4 <= N <= 16` 使用精确 `vector<Nxf32>` accumulator，覆盖 MNIST `N=10` 末层。
...
└── LiteNN/
    ...
    ├── Device/
    │   ├── CUDA.h                // CUDA DeviceTraits 声明（LITENN_ENABLE_CUDA）
...
├── CompiledModule.h/cpp         // Compiler<CPU/CUDA> + CompiledModule<CPU/CUDA>，object image + rodata/instruction loader
├── CUDANativePayload.h/cpp      // CUDA native instructions payload schema（PTX/cubin/fatbin + feature flags/scalar data + launch table）
├── Dump.h/cpp                   // MLIR stage dump + CompiledModule metadata dump
├── Dialect/
...
├── GraphDumpTest.cpp            // Graph textual dump 测试
├── CompilerTest.cpp             // Graph→MLIR 翻译 + verify 测试（需 LITENN_ENABLE_MLIR=ON）
├── CompilerDumpTest.cpp         // MLIR stage dump / metadata dump 测试（需 LITENN_ENABLE_MLIR=ON）
...
├── CompiledModuleCUDATest.cpp   // CUDA AOT bridge 测试（需 LITENN_ENABLE_MLIR=ON + LITENN_ENABLE_CUDA=ON）
└── LayerTest.cpp                // Softmax/GELU/ELU/LayerNorm 层正确性测试```
...
- [x] **内存安全验证**：新增 `LITENN_ENABLE_SANITIZERS` / `LITENN_SANITIZERS` CMake 开关；GNU/Clang 构建可启用 ASAN/UBSAN/LSAN，MSVC 构建回退到 AddressSanitizer + Debug CRT heap diff。新增 `MemorySafetyTest` 长时间循环覆盖 Tensor 视图赋值/析构、PolymorphicDevice 拷贝/张量 round-trip，以及 `CompiledModule::Load` 对 image 字节的复制与生命周期隔离。
...
- [x] **CompiledModule image ABI 版本化**：rodata 包含 magic、version、pointer size、endianness、target triple、backend 和命名 input/output specs；`Load` 会进行兼容性检查并拒绝 ABI 不匹配 image。CPU AOT 的 instructions 是 native object bytes；CUDA native 的 instructions 是 `CUDANativeInstructionPayload`（PTX/cubin/fatbin、feature flags、scalar data、launch table），详细演进见 `docs/CUDAAOTRoadmap.md`。
- [x] **静态库/动态库加载示例**：新增 `example/carrier`，构建时先生成 carrier object，再分别通过静态库 `extern` 符号和共享库 `GetProcAddress`/`dlsym` 导出的 `<prefix>_rodata{,_size}`、`<prefix>_instructions{,_size}` 恢复 `CompiledModuleArtifact` 并执行。
...
- [x] **对象文件与 JIT 路径分层**：新增 `CompiledModuleArtifact` 作为编译期产物层，`Compiler<CPU>::CompileArtifact` 负责生成 owning artifact 与 carrier object；`CompiledModule<CPU>::Load`/`artifact.Load()` 负责运行时 JIT 装载，MNIST AOT 与 carrier 示例均切到该分层。
- [x] **CompiledModule 生命周期**：接口与文档现在明确 `Load` 会复制 rodata/instruction bytes，原 image/carrrier library 可在 `Load` 返回后释放；`Run`/`RunInto`/`RunManyInto` 支持独立 buffer 并发；析构仍需与并发 `Run` 分离，并建议先于承载 LLVM 的插件/共享库卸载发生。

### P2：性能工程

- [x] **AOT f32 MatMul micro-kernel（阶段完成）**：当前 AOT codegen 已为 bufferized `f32` MatMul contraction 提供专用 lowering。宽输出走 `vector<16xf32>` row-tile 策略，窄输出 `4 <= N <= 16` 走 `vector<Nxf32>`；MatMulBiasAdd/MatMulBiasAddReLU 可融合到 micro-kernel store 路径。
- [ ] **CPU kernel 后端补全**：Conv/Reduce/Pooling 等核心算子仍接近 reference backend；后续需要接入 BLAS、oneDNN、OpenBLAS 或继续扩展手写 SIMD kernel。
- [ ] **内存规划器**：减少逐节点 Tensor 分配，支持 activation reuse、arena allocator、in-place 分析和临时 buffer 生命周期规划。
- [x] **Pass benchmark 驱动（基础完成）**：新增 `benchmark/bench.cpp` 的 `--use-runinto`、`--parallel-requests` / `--request-threads` 路径，以及 `benchmark/profile.cpp` 的 Run/RunInto/object 输出分析。仍需接入 CI/perf gate。
- [x] **AOT 优化策略（阶段完成）**：已完成 host CPU/features、LLVM O3 pipeline、fast-math、vector dialect lowering、MatMul shape-specialized micro-kernel、MatMulBiasAdd/ReLU 融合。
- [x] **CUDA native AOT MVP**：`Compiler<CUDA>` 已识别静态 shape、单 subgraph、`Float32` 单输入单输出 `UnaryOp::Negate/Abs/Sqrt/Exp/Log/Sin/Cos`、双输入单输出 `BinaryOp::Add/Subtract/Multiply/Divide/Max/Min`、同 rank 静态 broadcast 和二维 `Float32` MatMul；elementwise 生成 `CUDANative` PTX payload，MatMul 生成 cuBLAS library-call payload。`CompiledModule<CUDA>` 直接以 CUDA Tensor 指针 launch kernel 或调用 cuBLAS，不匹配图仍回退 CPU AOT bridge。
- [x] **CUDA PTX 模板收敛（R0/R2/R3 完成）**：elementwise/broadcast PTX 已从 `CompiledModule.cpp` 拆到独立 `CUDANativeCodegen` 模块；当前 `UnaryOp::Negate/Abs/Sqrt/Exp/Log/Sin/Cos`、same-shape `BinaryOp::Add/Subtract/Multiply/Divide/Max/Min` 与 same-rank broadcast binary 已改为 `OpBuilder` 结构化生成 MLIR `gpu.module`/`gpu.func`，再 lowering 到 NVVM/LLVM IR，由 NVPTX 后端生成 PTX 并接入 CUDA native payload。`Abs/Sqrt/Exp/Log/Sin/Cos/Max/Min` 采用 NVVM intrinsic 或等价 LLVM op，避免 standalone PTX 依赖 libdevice 外部符号；手写 PTX 暂作为旧 op fallback 保留，后续继续扩展 reduce/fusion。详细拆分见 `docs/CUDAAOTRoadmap.md` 的 P2.5。
- [ ] **AOT 优化策略（后续）**：继续补 K blocking、B panel packing、CPU feature aware tile policy、Graph/MLIR/ASM dump 和 regression stats。
- [ ] **性能基准套件**：和 NumPy/PyTorch/ONNX Runtime 在小模型、MLP、CNN、控制流图上的 correctness 和吞吐对比。

### P2：工程发布与维护

- [x] **CMake install/export/package**：已支持安装头文件/库、导出 `LiteNN::LiteNN` 与可选 `LiteNN::LiteNNCompiler` targets，并生成 `LiteNNConfig.cmake` / `LiteNNConfigVersion.cmake`。外部 `cmake/PackageSmokeTest` 已验证基础包可单独 `find_package(LiteNN)`，Compiler 组件需显式 `COMPONENTS Compiler` 并提供 LLVM/MLIR CMake 前缀。
- [x] **C++26 module 导出完善**：`LiteNN.ixx` 需要成为稳定入口或明确标记 experimental，避免 include/module 两套入口语义漂移。
- [x] **Graph/MLIR dump 工具**：新增 `LiteNN::Debug::DumpGraph`、`LiteNN::Debug::DumpMLIR(stage)` 和 `LiteNN::Debug::DumpCompiledModuleMetadata`。当前可输出 Graph textual dump、输入/Lowering/Bufferization/LLVM codegen 阶段的 MLIR，以及 artifact/loaded module metadata，便于调试和 issue 复现。
- [x] **版本与兼容策略**：新增 `CHANGELOG.md` 与 `docs/Versioning.md`，明确 pre-1.0 的 semver 约束、弃用策略、Graph model serialization / CompiledModule image 迁移规则和发布最小检查清单；CMake 包版本匹配在 `0.x` 阶段收紧到 same-minor compatibility。
- [x] **文档质量**：新增 `docs/APIGuide.md`、`docs/DesignConstraints.md`、`docs/PassGuide.md`、`docs/DeviceBackendGuide.md`、`docs/AOTDeployment.md`、`docs/Troubleshooting.md` 与 `docs/README.md`，补齐用户与维护者文档入口。
...
- [x] **FusionPass**：算子融合 Pass，支持 `MatMulBiasAdd`、`MatMulBiasAddReLU` 和 `ElementWiseChain` 融合模式；新增 `FusedOpNode` 持有 `FusionPattern` 标签 + body 子图；Interpreter 通过执行 body 保持语义等价；在 AutogradPass 之后运行
...
- **AOT Compiler\<D\>**：将 Graph 编译为设备特定可执行代码（`CompiledModule<D>`）
  ...
  - [x] **CUDA AOT bridge 第一版**：`Compiler<CUDA>::Compile/CompileArtifact` 与 `CompiledModule<CUDA>::Load/Run/RunInto/RunManyInto` 已打通，支持 CUDA Tensor 输入输出、rodata/instruction 地址加载和 artifact 加载；不匹配 native codegen 的图继续复用 CPU AOT object，再通过 CPU↔CUDA copy 做边界转换
  - [x] **CUDA AOT image backend 元数据**：rodata v3 新增 `CompiledModuleBackend`，并通过 `CompiledModuleArtifact::Backend()` / `CompiledModule<D>::Backend()` 暴露；CUDA native payload 额外记录 feature flags、scalar data、workspace/argument metadata
  - [x] **CUDA native runtime shell**：新增 `CUDANativeInstructionPayload`（PTX/cubin/fatbin + feature flags + scalar data + launch table + workspace/argument metadata）和 `CUDADriverModule`（CUDA Driver API module load/unload、kernel launch、默认/外部 stream、同步错误边界）
  - [x] **CUDA native loader + codegen MVP**：`CompiledModule<CUDA>` 可加载 `CUDANative` image，反序列化 payload、加载 Driver module 或识别 library-call，并按 launch table 执行；`Compiler<CUDA>` 已支持静态 shape、单 subgraph、`Float32` elementwise Negate/Abs/Sqrt/Exp/Log/Sin/Cos/Add/Subtract/Multiply/Divide/Max/Min、同 rank binary broadcast PTX codegen，以及二维 `Float32` MatMul/cuBLAS library-call
  - [ ] **CUDA 原生 AOT codegen 扩展**：继续扩大 MLIR GPU/NVVM lowering 覆盖面，补 reduce、自定义 MatMul/fusion epilogue、workspace allocator、stream/async 生命周期；详细分解见 `docs/CUDAAOTRoadmap.md`
- [ ] **JIT / Template Interpreter**：`Runtime/` 目录预留了扩展空间
- [x] **CUDA Device 第一版**：新增 `LITENN_ENABLE_CUDA`，提供 `CUDA` device、`CUDADeviceCount()` / `IsCUDADeviceAvailable()`、CUDA Runtime 分配/释放/清零、CPU↔GPU/D2D 拷贝、dtype 转换、`Float32/Float64` 二维 MatMul cuBLAS 路径，以及 unary/binary/reduce/concat/slice CPU fallback；新增 `CUDADeviceTest.cpp` 覆盖 round-trip、cuBLAS MatMul、跨实例 copy 与 PolymorphicDevice 路径
- [ ] **更多 Device**：Metal 等 GPU 后端；CUDA 后续需要补充原生 kernel、stream/allocator、异步执行与 AOT GPU codegen
