...
当前 unary、same-shape binary 与 same-rank broadcast binary CUDA native codegen 已验证 MLIR GPU/NVVM 路径，并已从手写 MLIR 文本模板改为 `OpBuilder` 结构化生成；手写 PTX 字符串模板仅保留为 fallback。模板的价值是验证 `CUDANativeInstructionPayload`、Driver module、launch table 和 CUDA Tensor 指针 ABI；长期目标是将剩余自定义 kernel 来源切到 MLIR GPU/NVVM 或更结构化的 kernel generator。
...
- [x] **R2：MLIR GPU/NVVM 试点**：新增可选实验路径，当前通过 `OpBuilder` 生成 `gpu.module`/`gpu.func` 表达最小 `f32` elementwise unary kernel，经 GPUToNVVM、LLVM IR、NVPTX 后端生成 PTX，并接入 `Compiler<CUDA>` unary payload；已覆盖 `UnaryOp::Negate/Abs/Sqrt`，其中 `Abs/Sqrt` 使用 NVVM intrinsic 避免 standalone PTX 依赖 libdevice 外部符号。
- [x] **R3：逐步替换模板**：unary、same-shape binary 与 same-rank broadcast binary 已接入 MLIR GPU/NVVM；手写 PTX 暂作为 fallback 保留，直到更大 CUDA native 覆盖面稳定。
...
- R2 完成后，MLIR/NVVM 试点必须是可选路径，失败时能回退到模板 codegen；当前已验证 `UnaryOp::Negate/Abs/Sqrt` 的 MLIR 生成 NVPTX PTX 可放入现有 payload 并通过 CUDA Driver 执行，避免同时改动 payload、runtime loader 和 broadcast 索引逻辑。
