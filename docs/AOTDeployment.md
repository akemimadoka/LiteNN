...
- `CompiledModule<Vulkan>`：Vulkan 运行期 module。`VulkanNative` artifact 的 instructions 是 SPIR-V payload bytes，
  不是平台 object file；移动端打包约束见 [VulkanMobileDeployment.md](VulkanMobileDeployment.md)。
...
- Vulkan native codegen 当前覆盖静态 shape、单 subgraph、同形 `Float32` unary/binary elementwise 和
  `Float32 <-> Int32` cast。`VulkanNative` instructions 必须作为 SPIR-V payload 解析，不走 CPU object loader。
