...
TEST(ModelIO, SaveLoadPreservesForwardBackwardAndVariables)
{
	...
	auto outputs = interpreter.RunForward(Detail::BuildExecutablePlanFromGraph(loaded), inputs);
	...
	auto gradients = interpreter.RunBackward(Detail::BuildExecutablePlanFromGraph(loaded), backwardInputs);
...
TEST(ModelIO, LoadRejectsPreVNextModelVersions)
{
	...
	EXPECT_THROW((void)Serialization::Detail::LoadGraphArchive(path), std::runtime_error);
	std::filesystem::remove(path);
}
...
TEST(ModelIO, SaveLoadPreservesFrozenVariablesWithoutGradientStorage)
{
	...
	std::vector<Tensor<CPU>> inputs;
	inputs.emplace_back(Tensor<CPU>({ 2.0f, 3.0f }, { 1, 2 }));
	const auto outputs = interpreter.RunForward(Detail::BuildExecutablePlanFromGraph(loaded), inputs);
	ASSERT_EQ(outputs.size(), 1);
...
TEST(ModelIO, SaveLoadExternalWeightsKeepsVariablesAlive)
{
	...
	Serialization::Detail::SaveGraphArchiveExternalWeights(graph, path, weightsPath, options);
