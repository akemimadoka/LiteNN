...
TEST(FusionPass, AfterAutogradPass)
{
	...
	auto expectedFwd = interpRef.RunForward(Detail::BuildExecutablePlanFromGraph(graphRef), inputsRef);
	...
	auto expectedBwd = interpRef.RunBackward(Detail::BuildExecutablePlanFromGraph(graphRef), bwdInputsRef);
	...
	auto actualFwd = interp.RunForward(Detail::BuildExecutablePlanFromGraph(graph), inputs);
