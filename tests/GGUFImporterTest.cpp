...
TEST(GGUFLLaMAQuantizedExecution, CompilesGroupedQ4KProjectionWithoutMaterializingConcatenatedWeight)
{
	...
	const auto compactArtifact = Compiler<CPU>::CompileArtifact(plan, prepackedOptions);
	EXPECT_TRUE(
	    ByteSpanContains(compactArtifact.Instructions(), "litenn_cpu_ggml_block_grouped_matmul3_compact_q8k_f32"));
	const auto compactWeightCount = std::ranges::count_if(compactArtifact.ExternalTensorInfos(), [](const auto& info) {
		return info.name.find(".prepacked.compact_block_grouped_v3.") != std::string::npos;
	});
	EXPECT_EQ(compactWeightCount, outFeatures.size());
	auto compactCompiled = compactArtifact.Load();
	const auto compactActual = compactCompiled.RunTensors(inputs);
...
TEST(GGUFLLaMAQuantizedExecution, CompilesGroupedKQuantProjectionToQ8KStagedHelper)
{
	...
	const auto runCase = [&](ggml_type ggmlType, QuantizedBlockFormat blockFormat, std::string_view name,
		...
		if (blockFormat == QuantizedBlockFormat::GGML_Q4_K || blockFormat == QuantizedBlockFormat::GGML_Q6_K)
		{
			...
			options.cpuAOTGGMLPrepackedWeightLayout = CPUAOTGGMLPrepackedWeightLayout::CompactBlockGroupedV3;
			const auto compactArtifact =
			    Compiler<CPU>::CompileArtifact(Detail::BuildExecutablePlanFromGraph(graph), options);
			const auto compactHelper = outFeatures.size() == 2
			                               ? "litenn_cpu_ggml_block_grouped_matmul2_compact_q8k_f32"
			                               : "litenn_cpu_ggml_block_grouped_matmul3_compact_q8k_f32";
			EXPECT_TRUE(ByteSpanContains(compactArtifact.Instructions(), compactHelper));
			const auto compactWeightCount =
			    std::ranges::count_if(compactArtifact.ExternalTensorInfos(), [](const auto& info) {
				    return info.name.find(".prepacked.compact_block_grouped_v3.") != std::string::npos;
			    });
			EXPECT_EQ(compactWeightCount, outFeatures.size());
			auto compactCompiled = compactArtifact.Load();
...
TEST(GGUFLLaMAQuantizedExecution, CompactInterleavedQ8KHelperMatchesStagedAndStaysNearRawSize)
{
	constexpr std::size_t inFeatures = 256;
	...
	for (const auto& [ggmlType, blockFormat, name] : cases)
	{
		...
		const auto& storage = quantizedWeight->Data();
		const auto* storageBytes = static_cast<const std::uint8_t*>(storage.UnsafeRawData());
		const auto compactBytes = litenn_cpu_ggml_compact_interleaved_bytes(static_cast<std::uint64_t>(blockFormat),
		                                                                    static_cast<std::int64_t>(outFeatures),
		                                                                    static_cast<std::int64_t>(inFeatures));
		ASSERT_GT(compactBytes, 64u);
		const auto layout = GetQuantizedBlockLayout(blockFormat);
		ASSERT_TRUE(layout.has_value());
		const auto paddedRawBytes =
		    ((outFeatures + 3) / 4) * 4 * (inFeatures / layout->elementsPerBlock) * layout->bytesPerBlock;
		EXPECT_EQ(compactBytes, 64u + paddedRawBytes);
		std::vector<std::uint8_t> compact(compactBytes);
		litenn_cpu_ggml_prepack_compact_interleaved(
		    ...
		    static_cast<std::uint64_t>(blockFormat), nullptr, compact.data(), 0,
		    static_cast<std::int64_t>(compact.size()), 1);

		std::vector<float> staged(rows * outFeatures);
		std::vector<float> compactOutput(rows * outFeatures);
		litenn_cpu_ggml_block_matmul_q8k_staged_f32(
		    nullptr, inputValues.data(), 0, static_cast<std::int64_t>(rows), static_cast<std::int64_t>(inFeatures),
		    static_cast<std::int64_t>(inFeatures), 1, nullptr, storageBytes, 0,
		    static_cast<std::int64_t>(storage.NumElements()), 1, nullptr, staged.data(), 0,
		...
		litenn_cpu_ggml_block_matmul_compact_q8k_f32(
		    nullptr, inputValues.data(), 0, static_cast<std::int64_t>(rows), static_cast<std::int64_t>(inFeatures),
		    static_cast<std::int64_t>(inFeatures), 1, nullptr, compact.data(), 0,
		    static_cast<std::int64_t>(compact.size()), 1, nullptr, compactOutput.data(), 0,
		    static_cast<std::int64_t>(rows), static_cast<std::int64_t>(outFeatures),
		    static_cast<std::int64_t>(outFeatures), 1, static_cast<std::uint64_t>(blockFormat), 2,
		    static_cast<std::uint64_t>(CPUAOTAffinityPolicy::None));
		...
		{
			EXPECT_NEAR(compactOutput[i], staged[i], 1.0e-4F);
		}
	}
}
...
TEST(GGUFLLaMAQuantizedExecution, FieldInterleavedV4Q8KHelperMatchesStagedWithFullAndTailGroups)
{
	...
	for (const auto outFeatures : outputWidths)
	{
		...
		for (const auto& [ggmlType, blockFormat, groupBlockBytes, name] : cases)
		{
			...
			if (outFeatures == outputWidths.front())
			{
				for (const std::uint64_t threadCount : { 4u, 1u, 3u, 2u, 4u })
				{
					std::ranges::fill(packedOutput, std::numeric_limits<float>::quiet_NaN());
					litenn_cpu_ggml_block_matmul_field_interleaved_v4_q8k_f32(
					    nullptr, inputValues.data(), 0, static_cast<std::int64_t>(rows),
					    static_cast<std::int64_t>(inFeatures), static_cast<std::int64_t>(inFeatures), 1, nullptr,
					    packed.data(), 0, static_cast<std::int64_t>(packed.size()), 1, nullptr, packedOutput.data(), 0,
					    static_cast<std::int64_t>(rows), static_cast<std::int64_t>(outFeatures),
					    static_cast<std::int64_t>(outFeatures), 1, static_cast<std::uint64_t>(blockFormat), threadCount,
					    static_cast<std::uint64_t>(CPUAOTAffinityPolicy::None));
					for (std::size_t i = 0; i < staged.size(); ++i)
					{
						EXPECT_NEAR(packedOutput[i], staged[i], 1.0e-4F)
						    << "with " << threadCount << " threads at element " << i;
					}
				}
			}
...
TEST(GGUFLLaMAQuantizedExecution, Q8KPreparedActivationHelperMatchesInternalStaging)
{
	...
	for (std::size_t i = 0; i < inputValues.size(); ++i)
	{
		inputValues[i] = static_cast<float>(static_cast<int>(i % 37) - 18) * 0.0625F;
	}
	...
	for (const auto& [ggmlType, blockFormat, name] : cases)
	{
		...
		const auto blockCount = inFeatures / GetQuantizedBlockLayout(blockFormat)->elementsPerBlock;
		...
		std::vector<float> internal(rows * outFeatures);
		std::vector<float> prepared(rows * outFeatures);
		litenn_cpu_ggml_block_matmul_q8k_staged_f32(
		    nullptr, inputValues.data(), 0, static_cast<std::int64_t>(rows), static_cast<std::int64_t>(inFeatures),
		    static_cast<std::int64_t>(inFeatures), 1, nullptr, storageBytes, 0,
		    static_cast<std::int64_t>(storage.NumElements()), 1, nullptr, internal.data(), 0,
		    static_cast<std::int64_t>(rows), static_cast<std::int64_t>(outFeatures),
		    static_cast<std::int64_t>(outFeatures), 1, static_cast<std::uint64_t>(blockFormat), 2,
		    static_cast<std::uint64_t>(CPUAOTAffinityPolicy::None));
		litenn_cpu_ggml_block_matmul_q8k_prepared_activation_f32(
		    ...
		    static_cast<std::int64_t>(rows), static_cast<std::int64_t>(inFeatures), nullptr, storageBytes, 0,
		    static_cast<std::int64_t>(storage.NumElements()), 1, nullptr, prepared.data(), 0,
		    static_cast<std::int64_t>(rows), static_cast<std::int64_t>(outFeatures),
		    static_cast<std::int64_t>(outFeatures), 1, static_cast<std::uint64_t>(blockFormat), 2,
		    static_cast<std::uint64_t>(CPUAOTAffinityPolicy::None));
...
TEST(GGUFLLaMAQuantizedExecution, Q6KPrepackedHelperMatchesDirectHelper)
{
	constexpr std::size_t inFeatures = 256;
	constexpr std::size_t outFeatures = 7;
	constexpr std::size_t rows = 3;
	...
	ASSERT_GT(preparedBlockBytes, 0u);
	const auto blockCount = inFeatures / GetQuantizedBlockLayout(QuantizedBlockFormat::GGML_Q6_K)->elementsPerBlock;
	std::vector<std::uint8_t> packed(outFeatures * blockCount * preparedBlockBytes);
	litenn_cpu_ggml_prepack_q6k_f32(nullptr, storageBytes, 0, static_cast<std::int64_t>(storage.NumElements()), 1,
	                                static_cast<std::int64_t>(outFeatures), static_cast<std::int64_t>(inFeatures),
	                                nullptr, packed.data(), 0, static_cast<std::int64_t>(packed.size()), 1);
	...
	litenn_cpu_ggml_block_matmul_f32(nullptr, inputValues.data(), 0, static_cast<std::int64_t>(rows),
	                                 ...
	                                 nullptr, direct.data(), 0, static_cast<std::int64_t>(rows),
	                                 static_cast<std::int64_t>(outFeatures), static_cast<std::int64_t>(outFeatures), 1,
	                                 static_cast<std::uint64_t>(QuantizedBlockFormat::GGML_Q6_K), 2,
	                                 static_cast<std::uint64_t>(CPUAOTAffinityPolicy::None));
	litenn_cpu_ggml_block_matmul_q6k_prepacked_f32(
	    nullptr, inputValues.data(), 0, static_cast<std::int64_t>(rows), static_cast<std::int64_t>(inFeatures),
	    static_cast<std::int64_t>(inFeatures), 1, nullptr, packed.data(), 0, static_cast<std::int64_t>(packed.size()),
...
TEST(GGUFLLaMACausalLM, CompilesBuilderStatefulDecodeScheduleWithLayerCheckpoints)
{
	...
	EXPECT_EQ(plan.outputs[1].name, "next_position");
	EXPECT_EQ(plan.outputs[2].name, "updated_key_0");
	EXPECT_EQ(plan.outputs[3].name, "updated_value_0");
	EXPECT_EQ(plan.outputs[4].name, "layer_hidden_0");
	EXPECT_EQ(projection.publicOutputIndices, std::vector<std::size_t>({ 0, 4 }));
	ASSERT_EQ(projection.stateAliases.size(), 3u);
	...
	auto artifact = Compiler<CPU>::CompileArtifact(schedule);
	ASSERT_EQ(artifact.OutputSpecs().size(), 2u);
	EXPECT_EQ(artifact.OutputSpecs()[0].name, "logits");
	EXPECT_EQ(artifact.OutputSpecs()[1].name, "layer_hidden_0");
	EXPECT_EQ(artifact.OutputSpecs()[1].type.StaticShape(), std::vector<std::size_t>({ 1, 4 }));
	auto compiled = artifact.Load();
	std::array<Tensor<CPU>, 4> inputs = {
		MakeInt32Tensor({ 1 }, { 1 }),
		MakeInt64Tensor({ 0 }, { 1 }),
	...
	const auto tolerance = GGUF::GetLLaMAParityTolerance(DataType::Float32);
	ExpectTensorNear(actual[0], expected[0], tolerance);
	ExpectTensorNear(actual[1], expected[4], tolerance);
	EXPECT_EQ(static_cast<const std::int64_t*>(inputs[1].UnsafeRawData())[0], 1);
	ExpectTensorNear(inputs[2], expected[2], tolerance);
	ExpectTensorNear(inputs[3], expected[3], tolerance);
}
