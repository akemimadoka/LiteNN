...
TEST(CompiledModuleCUDATest, ArtifactLoadsAsCUDABridge)
{
	...
	auto cpuOutput = outputs[0].CopyToDevice(CPU{});
	EXPECT_FLOAT_EQ(ReadFloat(cpuOutput, 0), 6.0f);
	EXPECT_FLOAT_EQ(ReadFloat(cpuOutput, 1), 8.0f);
	EXPECT_FLOAT_EQ(ReadFloat(cpuOutput, 2), 10.0f);
	EXPECT_FLOAT_EQ(ReadFloat(cpuOutput, 3), 12.0f);
}
