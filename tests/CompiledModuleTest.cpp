...
namespace
{
	...
	Graph BuildGroupedAffineDequantizeInputGraph()
	{
		Graph graph;
		Subgraph sg;
		auto params = GroupedAffineQuantization(DataType::Int8, 1, 2, { 0.5F, 0.25F, 2.0F, 1.0F, 0.125F, 4.0F },
		                                        { -1, 0, 2, 3, -4, 1 });
		const auto input = sg.AddParam(DataType::Int8, { 2, 5 });
		const auto dequantized = sg.AddNode(DequantizeNode{ { input, 0 }, params, DataType::Float32 },
		                                    { OutputInfo{ DataType::Float32, { 2, 5 } } });
		sg.SetResults({ { dequantized, 0 } });
		graph.AddSubgraph(std::move(sg));
		graph.SetForward(0);
		graph.SetInputNames({ "quantized" });
	...
	Graph BuildPerAxisAffineQuantizeInputGraph()
	{
		...
		const auto input = sg.AddParam(DataType::Float32, { 2, 3 });
		const auto quantized =
		    sg.AddNode(QuantizeNode{ { input, 0 }, params }, { OutputInfo{ DataType::Int8, { 2, 3 } } });
		sg.SetResults({ { quantized, 0 } });
		graph.AddSubgraph(std::move(sg));
		graph.SetForward(0);
		graph.SetInputNames({ "input" });
...
{
	auto graph = BuildSimpleAddGraph();
	auto separated = Compiler<CPU>::CompileArtifact(Detail::BuildExecutablePlanFromGraph(graph)).SeparateRodata();
...
TEST(CompiledModuleTest, CPUDynamicGroupedAffineDequantizeArtifactMatchesInterpreter)
{
	auto graph = BuildGroupedAffineDequantizeInputGraph();
	std::vector<Tensor<CPU>> inputs;
	inputs.emplace_back(Tensor<CPU>({ -1.0, 0.0, 2.0, 3.0, 5.0, 3.0, -4.0, 1.0, 7.0, -2.0 }, { 2, 5 }, DataType::Int8));
...
TEST(CompiledModuleTest, CPUDynamicPerAxisAffineQuantizeArtifactMatchesInterpreter)
{
	auto graph = BuildPerAxisAffineQuantizeInputGraph();
	std::vector<Tensor<CPU>> inputs;
	inputs.emplace_back(Tensor<CPU>({ -0.5, 0.125, 0.0, 1.0, -2.0, 8.0 }, { 2, 3 }, DataType::Float32));
...
TEST(CompiledModuleTest, CPUParallelLinearChainMatchesInterpreter)
{
	...
	{
		for (const auto waitPolicy :
		     { CPUAOTWorkerWaitPolicy::Adaptive, CPUAOTWorkerWaitPolicy::LowPower, CPUAOTWorkerWaitPolicy::Latency })
		{
			CompilerOptions options;
			options.cpuAOTThreadCount = 4;
			options.cpuAOTAffinityPolicy = affinityPolicy;
			options.cpuAOTWorkerWaitPolicy = waitPolicy;
...
TEST(CompiledModuleTest, CompilerDefaultsDoNotReadEnvironmentVariables)
{
	...
	auto graph = BuildWideLinearChainGraph(64);
	auto defaultArtifact = Compiler<CPU>::CompileArtifact(Detail::BuildExecutablePlanFromGraph(graph));
