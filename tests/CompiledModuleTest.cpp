...
TEST(CompiledModuleTest, CUDANativeMLIRNVPTXGeneratesNegatePTX)
{
	const auto ptx = CUDANativeUnaryF32PTXFromMLIRNVPTX(UnaryOp::Negate);

	EXPECT_NE(ptx.find("Generated by LLVM NVPTX Back-End"), std::string::npos);
	EXPECT_NE(ptx.find(".visible .entry litenn_negate_f32"), std::string::npos);
	EXPECT_NE(ptx.find("mov.u32"), std::string::npos);
	EXPECT_NE(ptx.find("neg.f32"), std::string::npos);
	...
	const auto payload = CUDANativeInstructionPayload{
	    ...
	    .kernels = {
	        CUDANativeKernelSpec{
	            .name = std::string(CUDANativeUnaryF32KernelName(UnaryOp::Negate)),
	            .grid = { .x = 1, .y = 1, .z = 1 },
	            .block = { .x = 4, .y = 1, .z = 1 },
	            .arguments = {
	                CUDANativeArgumentSpec{
	...
	EXPECT_EQ(decoded.binary.back(), std::byte{ 0 });
	ASSERT_EQ(decoded.kernels.size(), 1u);
	EXPECT_EQ(decoded.kernels[0].name, "litenn_negate_f32");
}
