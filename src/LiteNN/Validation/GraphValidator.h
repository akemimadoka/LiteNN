...
namespace LiteNN::Validation
{
	...
	private:
		                  ...
		                  const GroupedQuantizedMatMulNode& node) const
		{
			...
			{
				Fail(subgraphId, nodeId,
				     "GroupedQuantizedMatMulNode requires two or three rhs storages with matching projection metadata");
			}
			if (!IsFloatingDataType(lhs.dtype) || lhs.shape.size() != 2 || !node.transposeRhs)
			{
				Fail(subgraphId, nodeId,
			...
			for (std::size_t i = 0; i < node.rhsStorages.size(); ++i)
			{
				...
				const auto& projectionParams = node.projectionParams[i];
				if (projectionParams.expressedType != lhs.dtype ||
				    projectionParams.expressedShape != std::vector<std::size_t>{ node.outputWidths[i], lhs.shape[1] })
				{
					Fail(subgraphId, nodeId,
					     std::format("GroupedQuantizedMatMulNode projection {} expressed metadata is invalid", i));
				}
				try
				{
			...
			ExpectInfo(subgraphId, nodeId, entry.outputInfos[0], { lhs.dtype, { lhs.shape[0], totalOutputWidth } },
