...
namespace
{
	...
	AffineMap buildBroadcastMap(MLIRContext* ctx, ArrayRef<int64_t> inputShape, ArrayRef<int64_t> resultShape)
	{
	...
	Value emitBinaryScalar(OpBuilder& b, Location loc, BinaryOpKind kind, Value lhs, Value rhs, Type elemType)
	{
	...
	struct ConvertCallOp : OpRewritePattern<CallOp>
	{
		...
		{
			rewriter.replaceOpWithNewOp<func::CallOp>(op, op.getCallee(), op.getResultTypes(), op.getOperands());
			return success();
		}
	};
	...
	struct ConvertVariableOp : OpRewritePattern<VariableOp>
	{
		...
		LogicalResult matchAndRewrite(VariableOp op, PatternRewriter& rewriter) const override
		{
			...
			rewriter.replaceOpWithNewOp<memref::GlobalOp>(op, op.getSymName(),
	...
	struct ConvertBinaryOp : OpRewritePattern<litenn::BinaryOp>
	{
		...
		LogicalResult matchAndRewrite(litenn::BinaryOp op, PatternRewriter& rewriter) const override
		{
			...
			if (op.getOp() == BinaryOpKind::MatMul)
			{
				...
				auto filled =
				    rewriter.create<linalg::FillOp>(loc, ValueRange{ zero }, ValueRange{ emptyOut }).getResult(0);
				...
				auto generic = rewriter.create<linalg::GenericOp>(
				    loc, TypeRange{ resultType }, ValueRange{ lhs, rhs }, ValueRange{ filled }, maps, iterTypes,
				    [&](OpBuilder& b, Location l, ValueRange args) {
					    auto product = emitBinaryScalar(b, l, BinaryOpKind::Multiply, args[0], args[1], elemType);
			...
			auto emptyOut = rewriter.create<tensor::EmptyOp>(loc, resultType.getShape(), resultType.getElementType());
	...
	struct ConvertUnaryOp : OpRewritePattern<litenn::UnaryOp>
	{
		...
		LogicalResult matchAndRewrite(litenn::UnaryOp op, PatternRewriter& rewriter) const override
		{
			...
			if (op.getOp() == UnaryOpKind::Transpose)
			{
				...
				rewriter.replaceOpWithNewOp<linalg::TransposeOp>(op, input, emptyOut, ArrayRef<int64_t>(perm));
				return success();
			}
	...
	struct ConvertReduceOp : OpRewritePattern<litenn::ReduceOp>
	{
		...
		LogicalResult matchAndRewrite(litenn::ReduceOp op, PatternRewriter& rewriter) const override
		{
			...
			auto reduceOp = rewriter.create<linalg::ReduceOp>(
			    loc, ValueRange{ input }, ValueRange{ filledOut }, ArrayRef<int64_t>{ axis },
			    [&](OpBuilder& b, Location l, ValueRange args) {
			...
			if (kind == ReduceOpKind::Mean)
			{
				...
				if (isa<FloatType>(elemType))
				{
					...
					divisor =
					    rewriter.create<arith::ConstantFloatOp>(loc, ft, APFloat(ft.getFloatSemantics(), axisSize));
				}
				else
				{
	...
	struct ConvertConcatOp : OpRewritePattern<litenn::ConcatOp>
	{
		...
		{
			auto resultType = cast<RankedTensorType>(op.getResult().getType());
			rewriter.replaceOpWithNewOp<tensor::ConcatOp>(op, resultType, static_cast<int64_t>(op.getAxis()),
			                                              op.getInputs());
			return success();
		}
	};
	...
	struct ConvertSliceOp : OpRewritePattern<SliceOp>
	{
		...
		LogicalResult matchAndRewrite(SliceOp op, PatternRewriter& rewriter) const override
		{
			...
			rewriter.replaceOpWithNewOp<tensor::ExtractSliceOp>(op, dstType, input, ValueRange{}, ValueRange{},
	...
	struct ConvertCondOp : OpRewritePattern<CondOp>
	{
		...
		LogicalResult matchAndRewrite(CondOp op, PatternRewriter& rewriter) const override
		{
			...
			Value scalarCond = rewriter.create<tensor::ExtractOp>(loc, op.getCondition(), ValueRange{ c0 });
	...
	struct ConvertWhileOp : OpRewritePattern<WhileOp>
	{
		...
		LogicalResult matchAndRewrite(WhileOp op, PatternRewriter& rewriter) const override
		{
			...
			auto scfWhile = rewriter.create<scf::WhileOp>(loc, op.getResultTypes(), op.getInitArgs());
			...
			// created inside the correctly-parented scf::WhileOp before region.
			{
				...
				rewriter.replaceOpWithNewOp<scf::ConditionOp>(terminator, scalarCond, beforeBlock->getArguments());
			}
	...
	struct ConvertFusedOp : OpRewritePattern<FusedOp>
	{
		...
		LogicalResult matchAndRewrite(FusedOp op, PatternRewriter& rewriter) const override
		{
			    ...
			    op.getPattern() == FusionPatternKind::MatMulBiasAddReLU)
			{
				...
				if (!resultType || !lhsType || !rhsType || !biasType || resultType.getRank() != 2 ||
				...
				auto fused = rewriter.create<linalg::GenericOp>(
				    ...
				    ValueRange{ init.getResult(0) }, maps, iterTypes, [&](OpBuilder& b, Location l, ValueRange args) {
					    auto product = emitBinaryScalar(b, l, BinaryOpKind::Multiply, args[0], args[1], elemType);
					    auto sum = emitBinaryScalar(b, l, BinaryOpKind::Add, args[2], product, elemType);
					    b.create<linalg::YieldOp>(l, sum);
				    });
	...
	struct LowerLiteNNPass : PassWrapper<LowerLiteNNPass, OperationPass<ModuleOp>>
	{
		...
		{
			return "LowerLiteNNPass";
		}

		void getDependentDialects(DialectRegistry& registry) const override
		{
			registry.insert<arith::ArithDialect, bufferization::BufferizationDialect, func::FuncDialect,
			                linalg::LinalgDialect, math::MathDialect, memref::MemRefDialect, scf::SCFDialect,
		...
		void runOnOperation() override
		{
			...
			patterns.add<ConvertFuncOp, ConvertReturnOp, ConvertCallOp, ConvertConstantOp, ConvertVariableOp,
			             ConvertGetVariableOp, ConvertBinaryOp, ConvertUnaryOp, ConvertCastOp, ConvertReduceOp,
			             ConvertReshapeOp, ConvertConcatOp, ConvertSliceOp, ConvertCondOp, ConvertWhileOp,
			             ConvertYieldOp, ConvertFusedOp>(&getContext());
