...
namespace LiteNN
{
	namespace
	{
		...
		std::string_view AxisNormalizationStatsF32KernelName(NormalizationMode mode)
		{
			switch (mode)
			{
			case NormalizationMode::LayerNorm:
				return "layer_norm_stats";
			case NormalizationMode::RMSNorm:
				return "rms_norm_stats";
			default:
				throw std::runtime_error("Unsupported Vulkan native staged axis-normalization mode");
			}
		}

		std::string_view AxisNormalizationWriteF32KernelName(NormalizationMode mode)
		{
			switch (mode)
			{
			case NormalizationMode::LayerNorm:
				return "layer_norm_write";
			case NormalizationMode::RMSNorm:
				return "rms_norm_write";
			default:
				throw std::runtime_error("Unsupported Vulkan native staged axis-normalization mode");
			}
		}
		...
		mlir::spirv::FuncOp EmitSameShapeBinaryFunction(mlir::OpBuilder& moduleBuilder, mlir::Location loc,
		                                                DataType dtype, BinaryOp op, llvm::StringRef entryPoint,
		                                                std::uint32_t elementCount, mlir::spirv::GlobalVariableOp lhs,
		                                                mlir::spirv::GlobalVariableOp rhs,
		                                                mlir::spirv::GlobalVariableOp out,
		                                                mlir::spirv::GlobalVariableOp globalInvocationId)
		{
			...
			mlir::spirv::SelectionOp::createIfThen(
			    ...
			    [&](mlir::OpBuilder& bodyBuilder) {
				    ...
				    bodyBuilder.create<mlir::spirv::StoreOp>(
				        loc, EmitStorageBufferElementPointer(bodyBuilder, loc, valueType, out, index), result, nullptr,
				        nullptr);
			    },
			    moduleBuilder);
		...
		mlir::spirv::FuncOp EmitSameShapeUnaryFunction(mlir::OpBuilder& moduleBuilder, mlir::Location loc,
		                                               DataType dtype, UnaryOp op, llvm::StringRef entryPoint,
		                                               std::uint32_t elementCount, mlir::spirv::GlobalVariableOp input,
		                                               mlir::spirv::GlobalVariableOp out,
		                                               mlir::spirv::GlobalVariableOp globalInvocationId)
		{
			auto funcType = moduleBuilder.getFunctionType(mlir::TypeRange{}, mlir::TypeRange{});
		...
		mlir::OwningOpRef<mlir::spirv::ModuleOp> BuildSameShapeUnarySPIRVModule(DataType dtype, UnaryOp op,
		                                                                        std::uint32_t elementCount,
		                                                                        mlir::MLIRContext& context)
		{
			...
			auto func = EmitSameShapeUnaryFunction(moduleBuilder, loc, dtype, op, kEntryPointName, elementCount, input,
		...
		mlir::OwningOpRef<mlir::spirv::ModuleOp> BuildSameShapeBinarySPIRVModule(DataType dtype, BinaryOp op,
		                                                                         std::uint32_t elementCount,
		                                                                         mlir::MLIRContext& context)
		{
			mlir::OpBuilder builder(&context);
			...
			auto func = EmitSameShapeBinaryFunction(moduleBuilder, loc, dtype, op, kEntryPointName, elementCount, lhs,
			                                        rhs, out, globalInvocationId);
			moduleBuilder.setInsertionPointAfter(func);
			moduleBuilder.create<mlir::spirv::EntryPointOp>(
			    loc, mlir::spirv::ExecutionModel::GLCompute, func,
			    llvm::ArrayRef<mlir::Attribute>{ mlir::FlatSymbolRefAttr::get(globalInvocationId) });
		...
		{
			return BuildSameShapeBinarySPIRVModule(DataType::Float32, op, elementCount, context);
		}
		                                                                                 ...
		                                                                                 mlir::MLIRContext& context)
		{
			...
			for (const auto op : ops)
			{
				...
				auto func = EmitSameShapeBinaryFunction(moduleBuilder, loc, DataType::Float32, op, entryName,
				                                        elementCount, lhs, rhs, out, globalInvocationId);
				moduleBuilder.setInsertionPointAfter(func);
				moduleBuilder.create<mlir::spirv::EntryPointOp>(
				    loc, mlir::spirv::ExecutionModel::GLCompute, func,
				    llvm::ArrayRef<mlir::Attribute>{ mlir::FlatSymbolRefAttr::get(globalInvocationId) });
		...
		mlir::OwningOpRef<mlir::spirv::ModuleOp>
		BuildSameShapeElementwiseF32DAGSPIRVModule(std::span<const VulkanNativeElementwiseF32KernelOp> ops,
		                                           std::uint32_t elementCount, mlir::MLIRContext& context)
		{
			if (ops.empty())
			{
				throw std::runtime_error("Vulkan native elementwise DAG requires at least one op");
			}
			...
			std::vector<std::string> emittedEntryNames;
			for (const auto& op : ops)
			{
				std::string entryName;
				mlir::spirv::FuncOp func;
				switch (op.kind)
				{
				case VulkanNativeElementwiseF32KernelKind::Unary:
					entryName = SameShapeUnaryF32KernelName(op.unaryOp);
					if (llvm::is_contained(emittedEntryNames, entryName))
					{
						continue;
					}
					func = EmitSameShapeUnaryFunction(moduleBuilder, loc, DataType::Float32, op.unaryOp, entryName,
					                                  elementCount, lhs, out, globalInvocationId);
					break;
				case VulkanNativeElementwiseF32KernelKind::Binary:
					entryName = SameShapeBinaryF32KernelName(op.binaryOp);
					if (llvm::is_contained(emittedEntryNames, entryName))
					{
						continue;
					}
					func = EmitSameShapeBinaryFunction(moduleBuilder, loc, DataType::Float32, op.binaryOp, entryName,
					                                   elementCount, lhs, rhs, out, globalInvocationId);
					break;
				}
				...
				moduleBuilder.create<mlir::spirv::ExecutionModeOp>(
				    loc, func, mlir::spirv::ExecutionMode::LocalSize,
				    llvm::ArrayRef<int32_t>{ static_cast<int32_t>(kVulkanNativeElementwiseWorkgroupSize), 1, 1 });
				emittedEntryNames.push_back(std::move(entryName));
			}

			if (mlir::failed(mlir::verify(module)))
			{
				throw std::runtime_error(
				    "Generated Vulkan native MLIR SPIR-V elementwise DAG module verification failed");
			}
			return mlir::OwningOpRef<mlir::spirv::ModuleOp>(module);
		}
		...
		mlir::OwningOpRef<mlir::spirv::ModuleOp>
		BuildMatMulBiasF32SPIRVModule(std::span<const VulkanNativeMatMulBiasF32KernelSpec> specs,
		                              mlir::MLIRContext& context)
		{
			if (specs.empty())
			{
				throw std::runtime_error("Vulkan native MatMulBias module requires at least one kernel spec");
			}
			...
			{
				if (spec.kernelName.empty())
				{
					throw std::runtime_error("Vulkan native MatMulBias kernel entry point name must not be empty");
				}
				if (llvm::is_contained(emittedEntryNames, spec.kernelName))
				{
					throw std::runtime_error("Vulkan native MatMulBias kernel entry point names must be unique");
				}
				if (!VulkanNativeSupportsMatMulBiasF32(spec.m, spec.k, spec.n, spec.biasRows))
				{
					throw std::runtime_error("Vulkan native MatMulBias kernel spec has unsupported dimensions");
				}
				const auto outputElementCount = spec.m * spec.n;

				auto funcType = moduleBuilder.getFunctionType(mlir::TypeRange{}, mlir::TypeRange{});
				auto func = moduleBuilder.create<mlir::spirv::FuncOp>(loc, llvm::StringRef(spec.kernelName), funcType);
				auto* entry = moduleBuilder.createBlock(&func.getBody());
				moduleBuilder.setInsertionPointToStart(entry);
				...
				mlir::spirv::SelectionOp::createIfThen(
				    loc, inBounds,
				    [&](mlir::OpBuilder& bodyBuilder) {
					    auto nValue = EmitI32Constant(bodyBuilder, loc, spec.n);
					    auto row = bodyBuilder.create<mlir::spirv::UDivOp>(loc, index, nValue).getResult();
					    auto col = bodyBuilder.create<mlir::spirv::UModOp>(loc, index, nValue).getResult();
					    auto rowBase =
					        bodyBuilder.create<mlir::spirv::IMulOp>(loc, row, EmitI32Constant(bodyBuilder, loc, spec.k))
					            .getResult();
					    mlir::Value biasIndex = col;
					    if (spec.biasRows != 1)
					    {
					    ...
					    bodyBuilder.create<mlir::spirv::StoreOp>(
					        loc, EmitF32StorageBufferElementPointer(bodyBuilder, loc, out, index), sum, nullptr,
					        nullptr);
				    },
				    moduleBuilder);
				moduleBuilder.create<mlir::spirv::ReturnOp>(loc);
				...
				moduleBuilder.create<mlir::spirv::ExecutionModeOp>(
				    loc, func, mlir::spirv::ExecutionMode::LocalSize,
				    llvm::ArrayRef<int32_t>{ static_cast<int32_t>(kVulkanNativeMatMulWorkgroupSize), 1, 1 });
				emittedEntryNames.push_back(spec.kernelName);
			}

			if (mlir::failed(mlir::verify(module)))
			{
				throw std::runtime_error("Generated Vulkan native MLIR SPIR-V MatMulBias multi-entry module "
				                         "verification failed");
			}
			return mlir::OwningOpRef<mlir::spirv::ModuleOp>(module);
		}
		                                                                    ...
		                                                                    mlir::MLIRContext& context)
		{
			...
			const auto emitRowBase = [&](mlir::OpBuilder& b, mlir::Value rowIndex) {
				...
				return b
				    .create<mlir::spirv::IAddOp>(loc, b.create<mlir::spirv::IMulOp>(loc, outerAxis, inner).getResult(),
				                                 innerIndex)
				    .getResult();
			};
			const auto emitRowIndexFromElement = [&](mlir::OpBuilder& b, mlir::Value elementIndex) {
				...
				return b
				    .create<mlir::spirv::IAddOp>(loc, b.create<mlir::spirv::IMulOp>(loc, outerIndex, inner).getResult(),
				                                 innerIndex)
				    .getResult();
			};
			...
			auto funcType = moduleBuilder.getFunctionType(mlir::TypeRange{}, mlir::TypeRange{});
			auto rowMaxFunc = moduleBuilder.create<mlir::spirv::FuncOp>(loc, SoftmaxRowMaxF32KernelName(), funcType);
			...
			mlir::spirv::SelectionOp::createIfThen(
			    ...
			    [&](mlir::OpBuilder& bodyBuilder) {
				    ...
				    bodyBuilder.create<mlir::spirv::StoreOp>(
				        loc, EmitF32StorageBufferElementPointer(bodyBuilder, loc, rowMax, rowIndex), maxValue, nullptr,
				        nullptr);
			    },
			    moduleBuilder);
			moduleBuilder.create<mlir::spirv::ReturnOp>(loc);
			...
			auto rowSumFunc = moduleBuilder.create<mlir::spirv::FuncOp>(loc, SoftmaxRowSumF32KernelName(), funcType);
			...
			mlir::spirv::SelectionOp::createIfThen(
			    ...
			    [&](mlir::OpBuilder& bodyBuilder) {
				    ...
				    bodyBuilder.create<mlir::spirv::StoreOp>(
				        loc, EmitF32StorageBufferElementPointer(bodyBuilder, loc, rowSum, rowIndex), sum, nullptr,
				        nullptr);
			    },
			    moduleBuilder);
			moduleBuilder.create<mlir::spirv::ReturnOp>(loc);
		...
		mlir::OwningOpRef<mlir::spirv::ModuleOp>
		BuildAxisNormalizationF32SPIRVModule(NormalizationMode mode, std::span<const std::size_t> inputShape,
		                                     ...
		                                     mlir::MLIRContext& context)
		{
			...
			{
				throw std::runtime_error("Vulkan native staged axis-normalization supports only LayerNorm and RMSNorm");
			}
			const auto elementCount = NumElementsU32(inputShape);
			const auto innerSize = AxisInnerSizeU32(inputShape, axis);
			if (!elementCount || !innerSize || axis >= inputShape.size() || inputShape[axis] == 0 ||
			    inputShape[axis] > std::numeric_limits<std::uint32_t>::max())
			{
				throw std::runtime_error("Vulkan native staged axis-normalization shape is too large or empty");
			}
			const auto reductionSize = static_cast<std::uint32_t>(inputShape[axis]);
			const auto rowCount = *elementCount / reductionSize;
			const auto axisSpan = reductionSize * *innerSize;
			const bool layerNorm = mode == NormalizationMode::LayerNorm;
			...
			std::optional<mlir::spirv::GlobalVariableOp> rowMean;
			if (layerNorm)
			{
				rowMean = CreateStorageBuffer(moduleBuilder, loc, bufferStruct, "row_mean", nextBinding++);
			}
			auto rowDenom = CreateStorageBuffer(moduleBuilder, loc, bufferStruct, "row_denom", nextBinding++);
			...
			const auto emitRowBase = [&](mlir::OpBuilder& b, mlir::Value rowIndex) {
				auto inner = EmitI32Constant(b, loc, *innerSize);
				auto axisValue = EmitI32Constant(b, loc, reductionSize);
				auto outerIndex = b.create<mlir::spirv::UDivOp>(loc, rowIndex, inner).getResult();
				auto innerIndex = b.create<mlir::spirv::UModOp>(loc, rowIndex, inner).getResult();
				auto outerAxis = b.create<mlir::spirv::IMulOp>(loc, outerIndex, axisValue).getResult();
				return b
			...
			const auto loadInput = [&](mlir::OpBuilder& b, mlir::Value offset) {
				return b
				    .create<mlir::spirv::LoadOp>(loc, b.getF32Type(),
				                                 EmitF32StorageBufferElementPointer(b, loc, input, offset), nullptr,
				                                 nullptr)
				    .getValue();
			};
			const auto emitMemberOffset = [&](mlir::OpBuilder& b, mlir::Value base, std::uint32_t reduceIndex) {
				auto inner = EmitI32Constant(b, loc, *innerSize);
				return b
				    .create<mlir::spirv::IAddOp>(
				        loc, base,
				        b.create<mlir::spirv::IMulOp>(loc, EmitI32Constant(b, loc, reduceIndex), inner).getResult())
				    .getResult();
			};
			const auto emitAxisIndexFromElement = [&](mlir::OpBuilder& b, mlir::Value elementIndex) {
				auto inner = EmitI32Constant(b, loc, *innerSize);
				auto axisValue = EmitI32Constant(b, loc, reductionSize);
				return b
				    .create<mlir::spirv::UModOp>(
				        loc, b.create<mlir::spirv::UDivOp>(loc, elementIndex, inner).getResult(), axisValue)
				    .getResult();
			};

			auto funcType = moduleBuilder.getFunctionType(mlir::TypeRange{}, mlir::TypeRange{});
			auto statsFunc =
			    moduleBuilder.create<mlir::spirv::FuncOp>(loc, AxisNormalizationStatsF32KernelName(mode), funcType);
			auto* entry = moduleBuilder.createBlock(&statsFunc.getBody());
			moduleBuilder.setInsertionPointToStart(entry);
			auto rowIndex = EmitGlobalInvocationIndex(moduleBuilder, loc, globalInvocationId);
			auto inBounds = EmitElementwiseInBounds(moduleBuilder, loc, rowIndex, rowCount);
			mlir::spirv::SelectionOp::createIfThen(
			    ...
			    [&](mlir::OpBuilder& bodyBuilder) {
				    ...
				    for (std::uint32_t reduceIndex = 0; reduceIndex < reductionSize; ++reduceIndex)
				    {
					    auto value = loadInput(bodyBuilder, emitMemberOffset(bodyBuilder, base, reduceIndex));
					    auto centered =
					        layerNorm ? bodyBuilder.create<mlir::spirv::FSubOp>(loc, value, mean).getResult() : value;
			    ...
			    moduleBuilder);
			moduleBuilder.create<mlir::spirv::ReturnOp>(loc);
			emitEntryPoint(statsFunc);

			auto writeFunc =
			    moduleBuilder.create<mlir::spirv::FuncOp>(loc, AxisNormalizationWriteF32KernelName(mode), funcType);
			entry = moduleBuilder.createBlock(&writeFunc.getBody());
			moduleBuilder.setInsertionPointToStart(entry);
			auto outputIndex = EmitGlobalInvocationIndex(moduleBuilder, loc, globalInvocationId);
			inBounds = EmitElementwiseInBounds(moduleBuilder, loc, outputIndex, *elementCount);
			mlir::spirv::SelectionOp::createIfThen(
			    ...
			    [&](mlir::OpBuilder& bodyBuilder) {
				    auto rowIndexForElement = emitRowIndexFromElement(bodyBuilder, outputIndex);
				    auto current = loadInput(bodyBuilder, outputIndex);
				    mlir::Value centered = current;
				    if (layerNorm)
				    {
					    auto mean =
					        bodyBuilder
					            .create<mlir::spirv::LoadOp>(
					                loc, bodyBuilder.getF32Type(),
					                EmitF32StorageBufferElementPointer(bodyBuilder, loc, *rowMean, rowIndexForElement),
					                nullptr, nullptr)
					            .getValue();
			...
			{
				throw std::runtime_error(
				    "Generated Vulkan native MLIR SPIR-V staged axis-normalization module verification failed");
			}
			return mlir::OwningOpRef<mlir::spirv::ModuleOp>(module);
		}
		...
		VulkanNativeGeneratedSPIRV SerializeSameShapeBinarySPIRV(DataType dtype, BinaryOp op,
		                                                         std::uint32_t elementCount)
		{
			mlir::MLIRContext context;
			context.getOrLoadDialect<mlir::spirv::SPIRVDialect>();

			auto module = BuildSameShapeBinarySPIRVModule(dtype, op, elementCount, context);
			ValidateVulkanShaderModule(module.get());
		...
		{
			return SerializeSameShapeBinarySPIRV(DataType::Float32, op, elementCount);
		}
		...
		SerializeSameShapeElementwiseF32DAGSPIRV(std::span<const VulkanNativeElementwiseF32KernelOp> ops,
		                                         std::uint32_t elementCount)
		{
			mlir::MLIRContext context;
			context.getOrLoadDialect<mlir::spirv::SPIRVDialect>();

			std::vector<std::string> uniqueEntryNames;
			for (const auto& op : ops)
			{
				std::string entryName;
				switch (op.kind)
				...
				if (!llvm::is_contained(uniqueEntryNames, entryName))
				{
					uniqueEntryNames.push_back(std::move(entryName));
				}
			}
			auto module = BuildSameShapeElementwiseF32DAGSPIRVModule(ops, elementCount, context);
			ValidateVulkanShaderModule(module.get(), uniqueEntryNames.size());
			...
			{
				throw std::runtime_error(
				    "Failed to serialize generated Vulkan native MLIR SPIR-V elementwise DAG module");
			}
		...
		VulkanNativeGeneratedSPIRV
		SerializeMatMulBiasF32SPIRV(std::span<const VulkanNativeMatMulBiasF32KernelSpec> specs)
		{
			mlir::MLIRContext context;
			context.getOrLoadDialect<mlir::spirv::SPIRVDialect>();

			auto module = BuildMatMulBiasF32SPIRVModule(specs, context);
			ValidateVulkanShaderModule(module.get(), specs.size());
			...
			{
				throw std::runtime_error(
				    "Failed to serialize generated Vulkan native MLIR SPIR-V MatMulBias multi-entry module");
			}
		...
		VulkanNativeGeneratedSPIRV SerializeAxisNormalizationF32SPIRV(NormalizationMode mode,
		                                                              std::span<const std::size_t> inputShape,
		                                                              ...
		                                                              bool hasBias)
		{
			...
			auto module =
			    BuildAxisNormalizationF32SPIRVModule(mode, inputShape, axis, epsilon, hasScale, hasBias, context);
			ValidateVulkanShaderModule(module.get(), 2);
			...
			{
				throw std::runtime_error(
				    "Failed to serialize generated Vulkan native MLIR SPIR-V staged axis-normalization module");
			}
	...
	std::string VulkanNativeSameShapeUnaryF32KernelName(UnaryOp op)
	{
		...
		return SameShapeUnaryF32KernelName(op);
	}
	...
	bool VulkanNativeSupportsSameShapeBinary(DataType dtype, BinaryOp op)
	{
		...
		return VulkanNativeSupportsSameShapeBinaryF32(op);
	}

	VulkanNativeGeneratedSPIRV VulkanNativeSameShapeBinaryF32SPIRV(BinaryOp op, std::uint32_t elementCount)
	{
		return VulkanNativeSameShapeBinarySPIRV(DataType::Float32, op, elementCount);
	}

	VulkanNativeGeneratedSPIRV VulkanNativeSameShapeBinarySPIRV(DataType dtype, BinaryOp op, std::uint32_t elementCount)
	{
		if (!VulkanNativeSupportsSameShapeBinary(dtype, op))
		{
		...
		return SerializeSameShapeBinarySPIRV(dtype, op, elementCount);
	}

	std::string VulkanNativeSameShapeBinaryF32KernelName(BinaryOp op)
	{
		return VulkanNativeSameShapeBinaryKernelName(DataType::Float32, op);
	}
	                                            ...
	                                            std::uint32_t elementCount)
	{
		if (ops.empty())
		{
			throw std::runtime_error("Vulkan native same-shape elementwise DAG requires at least one op");
		}
		if (elementCount == 0)
		{
			throw std::runtime_error("Vulkan native same-shape elementwise DAG requires a non-empty static shape");
		}
		for (const auto& op : ops)
		{
			switch (op.kind)
			{
			case VulkanNativeElementwiseF32KernelKind::Unary:
				if (!VulkanNativeSupportsSameShapeUnaryF32(op.unaryOp))
				{
					throw std::runtime_error("Unsupported Vulkan native same-shape f32 unary DAG op");
				}
				break;
			case VulkanNativeElementwiseF32KernelKind::Binary:
				if (!VulkanNativeSupportsSameShapeBinaryF32(op.binaryOp))
				{
					throw std::runtime_error("Unsupported Vulkan native same-shape f32 binary DAG op");
				}
				break;
			}
		}
		return SerializeSameShapeElementwiseF32DAGSPIRV(ops, elementCount);
	}
	...
	VulkanNativeMatMulBiasF32SPIRV(std::span<const VulkanNativeMatMulBiasF32KernelSpec> specs)
	{
		if (specs.empty())
		{
			throw std::runtime_error("Vulkan native MatMulBias multi-entry module requires at least one spec");
		}
		for (const auto& spec : specs)
		{
			if (spec.kernelName.empty())
			{
				throw std::runtime_error("Vulkan native MatMulBias multi-entry spec requires a kernel name");
			}
			if (!VulkanNativeSupportsMatMulBiasF32(spec.m, spec.k, spec.n, spec.biasRows))
			{
				throw std::runtime_error("Vulkan native MatMulBias multi-entry spec contains unsupported dimensions");
			}
		}
		return SerializeMatMulBiasF32SPIRV(specs);
	}
	                                                                 ...
	                                                                 bool hasBias)
	{
		...
		if (!VulkanNativeSupportsNormalizationF32(mode, inputShape, axis, 1))
		{
			throw std::runtime_error("Vulkan native staged axis-normalization requires static non-empty shape and an "
			                         "in-range axis");
		}
		return SerializeAxisNormalizationF32SPIRV(mode, inputShape, axis, epsilon, hasScale, hasBias);
	}

	std::string_view VulkanNativeAxisNormalizationStatsF32KernelName(NormalizationMode mode)
	{
		return AxisNormalizationStatsF32KernelName(mode);
	}

	std::string_view VulkanNativeAxisNormalizationWriteF32KernelName(NormalizationMode mode)
	{
		return AxisNormalizationWriteF32KernelName(mode);
	}
