...
namespace LiteNN::Layer
{
	inline NodeOutput AddPool2D(Subgraph& subgraph, NodeOutput input, PoolMode mode,
	                            ...
	                            std::vector<std::size_t> highPads = { 0, 0 }, bool countIncludePad = false)
	{
		...
		const auto outputShape =
		    ::LiteNN::Detail::Pool2DOutputShape(info.shape, kernelShape, strides, lowPads, highPads);
		const auto result = subgraph.AddNode(Pool2DNode{ input, mode, std::move(kernelShape), std::move(strides),
		                                                 std::move(lowPads), std::move(highPads), countIncludePad },
		                                     { OutputInfo{ info.dtype, outputShape } });
		return { result, 0 };
	}

	inline NodeOutput AddMaxPool2D(Subgraph& subgraph, NodeOutput input, std::vector<std::size_t> kernelShape,
	                               std::vector<std::size_t> strides, std::vector<std::size_t> lowPads = { 0, 0 },
	                               std::vector<std::size_t> highPads = { 0, 0 })
	{
		return AddPool2D(subgraph, input, PoolMode::Max, std::move(kernelShape), std::move(strides), std::move(lowPads),
		                 std::move(highPads));
	}

	inline NodeOutput AddAveragePool2D(Subgraph& subgraph, NodeOutput input, std::vector<std::size_t> kernelShape,
	                                   std::vector<std::size_t> strides, std::vector<std::size_t> lowPads = { 0, 0 },
	                                   std::vector<std::size_t> highPads = { 0, 0 }, bool countIncludePad = false)
	{
