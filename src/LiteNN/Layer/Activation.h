...
namespace LiteNN::Layer
{
	inline namespace Activation
	{
		...
		inline NodeOutput AddSigmoid(Subgraph& subgraph, NodeOutput input)
		{
			...
			const auto neg =
			    subgraph.AddNode(UnaryOpNode{ UnaryOp::Negate, input }, { OutputInfo{ info.dtype, info.shape } });
			const auto exp =
		...
		inline NodeOutput AddGELUErf(Subgraph& subgraph, NodeOutput input)
		{
			...
			const auto scaled = subgraph.AddNode(BinaryOpNode{ BinaryOp::Multiply, input, { invSqrt2, 0 } },
			                                     { OutputInfo{ info.dtype, info.shape } });
			const auto erf =
			    subgraph.AddNode(UnaryOpNode{ UnaryOp::Erf, { scaled, 0 } }, { OutputInfo{ info.dtype, info.shape } });
		...
		inline SubgraphId BuildClamp(ModelBuilder& builder, DataType dtype, ShapeView shape, double minValue,
		                             double maxValue)
		{
			Subgraph subgraph;
			const auto input = subgraph.AddParam(dtype, shape.ToOwned());
		...
		inline SubgraphId BuildLeakyReLU(ModelBuilder& builder, DataType dtype, ShapeView shape,
