...
namespace
{
	...
	void BMGGMLCompactInterleavedMatMulHelper(benchmark::State& state, QuantizedBlockFormat format, std::size_t batch,
	                                          ...
	                                          std::uint64_t threadCount)
	{
		...
		if (compactBytes == 0)
		{
			state.SkipWithError("unsupported compact GGML benchmark shape");
			return;
		}
		...
		litenn_cpu_ggml_prepack_compact_interleaved(
		    ...
		    static_cast<std::int64_t>(inputWidth), static_cast<std::uint64_t>(format), nullptr, compact.data(), 0,
		...
		const auto invoke = [&] {
			litenn_cpu_ggml_block_matmul_compact_q8k_f32(
			    nullptr, lhs.data(), 0, static_cast<std::int64_t>(batch), static_cast<std::int64_t>(inputWidth),
			    static_cast<std::int64_t>(inputWidth), 1, nullptr, compact.data(), 0,
			    static_cast<std::int64_t>(compact.size()), 1, nullptr, output.data(), 0,
		...
		state.counters["compact_bytes"] = benchmark::Counter(static_cast<double>(compact.size()));
	...
	void RegisterBenchmarks()
	{
		...
		constexpr std::array ggmlCompactInterleavedBlockFormats{
			QuantizedBlockFormat::GGML_Q4_K,
			QuantizedBlockFormat::GGML_Q6_K,
		};
